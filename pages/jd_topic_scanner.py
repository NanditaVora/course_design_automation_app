...
def process_query(uploaded_file, scan_topic, experience):
    vectorstore = initialize_vector_store(uploaded_file)
    query = f"""
        List down the most popular topics related to {scan_topic} which are in demand by the employers from the candidates whose experience level is {experience}.
        The topics should be relevant to the audience who will be freshly entering the job market.
        ...
        •	Advanced Database Topics
            ...
            o	Transactions
            o	Indexing
        Please ensure the list covers only the topics having the mention of the given subject {scan_topic}.Exclude all the rest of the topics
...
def handle_get_response(uploaded_file, scan_topic, experience):

    # Get Response Button
    if st.button('Get Response'):
        if uploaded_file and scan_topic:
            response = process_query(uploaded_file, scan_topic, experience)
            st.session_state.jd_response = response
            return response
        else:
...
def get_jd_files(directory):
    return [f for f in os.listdir(directory) if f.endswith('.csv')]

# Function to design dropdown for job description dump files
def create_file_selector(jd_directory,inputbox):

    # Get list of JD dump files
    jd_files = get_jd_files(jd_directory)

    # Dropdown to select a PDF file
    selected_jd = inputbox.selectbox("Select JD dump file for Scanning", jd_files)
    return selected_jd
...
def main():
    ...
    with st.sidebar:
        ...
        jd_directory = "inputs/jds"

        # Create dropdown for jd dump files
        jd_dump_file = create_file_selector(jd_directory, inputbox)
        ...
        handle_get_response(jd_dump_file, scan_topic, experience)

    if "jd_response" not in st.session_state:
        initalize_session_state()
    if st.session_state.jd_response:
        st.header("Response")
        st.markdown(st.session_state.jd_response)
        # Save Response Button
        if st.button("Generate CTKS"):
            ctks_prompt = f"""
                Instructions for Generating Course Topic Knowledge Structure (CTKS):
                
                Input:
                - Scan Topic: {scan_topic}
                - Available Topics: {st.session_state.jd_response}
